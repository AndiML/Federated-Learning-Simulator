"""A module, which contains the client for FedProx."""
import copy
...
class ClientFedProx(object):
    ...
    def __init__(
        ...
        Args:
            ...
            proximal_weighting (float): The weighting of the regularizer in fedprox objective.
        ...
        self.proximal_weighting = proximal_weighting
    ...
    def train(self, global_round: int) -> tuple[float, list[float]]:
        ...
        old_global_model = copy.deepcopy(self.model)
        old_model_parameters = list(old_global_model.parameters())
        ...
        for current_epoch in range(self.local_epochs):
            ...
            for (images, labels) in progress_bar_training_data_loader:
                ...
                for param_index, param in enumerate(self.model.parameters()):
                    fed_prox_reg += ((self.proximal_weighting / 2) * torch.norm((param - old_model_parameters[param_index]))**2)
                loss += fed_prox_reg
