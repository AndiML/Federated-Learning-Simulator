...
from federated_learning.src.central_server_fednova import FederatedLearningFedNovaCentralServer
from federated_learning.src.saving import ModelSaver


def perform_fednova(
    central_server: FederatedLearningFedNovaCentralServer,
    eval_every: int,
    ...
    """ Performs the averaging of the local models retrieved from the participating clients
        ...
        Args:
            central_server (FederatedLearningFedNovaCentralServer): The central server which updates and validates the locally trained models of the
                clients.
    ...
    print(f'Global Loss: {central_server_loss[-1]:.5f}, Global Best Loss: {numpy.min(central_server_loss):.5f}')
    print(f'Global Validation Accuracy: {central_server_accuracy[-1]:.4f}, Global Best Validation Accuracy: {numpy.max(central_server_accuracy):.4f}')
    ...
    experiment_logger.add_metric('Max Difference', 'Max Model Difference', 0)
    ...
    for communication_round in range(number_of_training_rounds):
        ...
        client_to_normalization_constant = []
        client_to_normalized_global_update = []
        ...
        for client_index in sampled_client_indices:
            ...
            _, a_i, global_model_parameter_normalized = clients_list[client_index].train(communication_round)
            client_to_normalization_constant.append(a_i)
            client_to_normalized_global_update.append(global_model_parameter_normalized)

        # Updates central server model and validates the central server model
        central_server.update_global_model(
                sampled_client_indices.tolist(),
                client_to_normalization_constant,
                client_to_normalized_global_update,
            )
