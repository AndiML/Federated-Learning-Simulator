"""A module, which contains the client."""
import copy
...
class ClientFedEntropy(object):
    ...
    def train(self, global_round: int) -> tuple[float, list[float]]:
        ...
        Returns:
            tuple[float, list[float]]: The average loss over all local training epochs and a list encoding the average loss for each epoch.
        Raises:
            ValueError: If the specified optimizer or loss function are not supported, an exception is raised.
    ...
    @torch.no_grad()
    def retrieve_soft_labels_of_trained_client_model(self) -> torch.Tensor:
        """
        Computes the average soft labels over the entire training dataset for the client's model.

        Returns:
            torch.Tensor: A tensor representing the averaged soft labels for each class.
        """
        ...
        # Creates the data loader for the training set
        local_training_test_data_loader = self.data_class_instance.get_training_data_loader(
            batch_size=self.local_batchsize,
            client_id=self.client_id,
            shuffle_samples=False
        ...
        # Wraps the training data loader with tqdm to create a progress bar
        progress_bar_test_data_loader = tqdm(
            local_training_test_data_loader,
            total=len(local_training_test_data_loader),
            desc="Compute Soft Labels over the whole training set: "
        )

        soft_label = None

        # Performs inference and calculates probabilities for each training sample
        for (images, labels) in progress_bar_test_data_loader:
            images = images.to(self.device)
            labels = labels.to(self.device)
            log_softmax = self.model(images)

            # Converts log probabilities to real probabilities
            probabilities = torch.exp(log_softmax)

            # Aggregating probabilities for soft labels
            if soft_label is None:
                soft_label = probabilities
            else:
                soft_label = torch.cat((soft_label, probabilities), 0)

        # Calculate the mean of probabilities across all batches
        soft_label = torch.mean(soft_label, dim=0)
