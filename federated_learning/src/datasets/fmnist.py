...
class Fmnist(Dataset):
    ...
    def __init__(self, path: str) -> None:
        ...
        training_data = torchvision.datasets.FashionMNIST(root=self.path, train=True, download=True, transform=transform)
        self.labels: list[int] = training_data.targets
        self._training_data: DatasetData = training_data
        self._validation_data: DatasetData = torchvision.datasets.FashionMNIST(root=self.path, train=False, download=True, transform=transform)
    ...
    def download(path: str) -> None:
        """ Downloads the fmnist dataset to the specified directory.

        Args:
            path(str): The path to the directory into which the fmnist dataset is to be downloaded.
        """
        torchvision.datasets.FashionMNIST(root=path, download=True)
