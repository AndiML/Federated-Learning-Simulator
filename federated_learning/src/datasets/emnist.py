"""Represents a module containing FMNIST or FMNIST-style datasets."""
...
class Emnist(Dataset):
    ...
    def __init__(self, path: str) -> None:
        ...
        transform = torchvision.transforms.Compose([
            ...
            torchvision.transforms.Normalize((0.1736,), (0.3317,))
        ])
        # Loads the training and validation splits of the dataset and extracts their labels
        training_data = torchvision.datasets.EMNIST(root=self.path, split='byclass', train=True, download=True, transform=transform)
        self.labels: list[int] = training_data.targets
        self._training_data: DatasetData = training_data
        self._validation_data: DatasetData = torchvision.datasets.EMNIST(root=self.path, split='byclass', train=False, download=True, transform=transform)
    ...
    def download(path: str) -> None:
        """ Downloads the emnist dataset to the specified directory.

        Args:
            path(str): The path to the directory into which the fmnist dataset is to be downloaded.
        """
        torchvision.datasets.EMNIST(root=path, download=True)
