...
class Dataset(ABC):
    ...
    def __init__(self, labels: list[int]) -> None:
        ...
        self._partitioned_training_data: list[DatasetSplit] = []
        self._partitioned_training_data_per_label: list[list[DatasetSplit]] = []
        self._client_index_to_available_labels: list[int] = []
    ...
    @property
    def partitioned_training_data(self) -> list[DatasetSplit]:
        """Gets the training data of the dataset.
        ...
        return self._partitioned_training_data

    @property
    def client_index_to_available_labels(self) -> list[int]:
        """Retrieves a list to map each client index to the number of its available labels.

        Returns:
            list[int]: Returns the map each client index to the number of its available labels.
        """

        return self._client_index_to_available_labels

    @property
    def partitioned_training_data_per_label(self) -> list[list[DatasetSplit]]:
        """Gets the training data for each client, where the training indices are split up in their respective classes, and the available labels for
            client.

        Returns:
            list[list[DatasetSplit]]: Returns the training samples indices for each class.
        """

        return self._partitioned_training_data_per_label

    def get_training_data_loader(self, batch_size: int, client_id: int, shuffle_samples: bool) -> torch.utils.data.DataLoader:
        """Creates a data loader for the partitioned training data of the dataset.

        Args:
            batch_size (int): The number of samples per batch.
            client_id (int): The id of the client to retrieve the partitioned training data set.
        ...
        return create_data_loader(self.partitioned_training_data[client_id], batch_size, shuffle_samples=shuffle_samples)
    ...
    def get_validation_data_loader(self, batch_size: int) -> torch.utils.data.DataLoader:
        ...
        return create_data_loader(self.validation_data, batch_size, shuffle_samples=True)
