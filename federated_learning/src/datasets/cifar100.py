...
#  from federated_learning.src.datasets.data_generator import DatasetSplit
...
class Cifar100(Dataset):
    ...
    def __init__(self, path: str) -> None:
        ...
        training_data = torchvision.datasets.CIFAR100(root=self.path, train=True, download=True, transform=transform)
        self.labels: list[int] = training_data.targets
        self._training_data: DatasetData = training_data
        self._validation_data: DatasetData = torchvision.datasets.CIFAR100(root=self.path, train=False, download=True, transform=transform)
