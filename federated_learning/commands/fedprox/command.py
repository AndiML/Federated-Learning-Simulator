...
from federated_learning.src.fedprox import perform_fedprox


class FedProxCommand(BaseCommand):
    """Represents a command that represents the federated averaging with regularization algorithm."""
    ...
    def run(self, command_line_arguments: Namespace) -> None:
        ...
        for run in range(command_line_arguments.number_of_runs):
            ...
            experiment_logger.add_model_graph(global_model, torch.ones(command_line_arguments.global_batchsize, *data_class_instance.sample_shape))
            ...
            for client_id in range(command_line_arguments.number_of_clients):
                ...
                clients_list.append(
                    ClientFedProx(
                        ...
                        proximal_weighting=command_line_arguments.set_proximal_parameter,
                        device=device,
            ...
            average_train_loss, average_train_accuracy, central_server_loss, central_server_accuracy = perform_fedprox(
            ...
            all_average_train_loss.append(average_train_loss)
            all_average_train_accuracy.append(average_train_accuracy)
        ...
        all_average_train_loss = numpy.mean(all_average_train_loss, axis=0)
        all_average_train_accuracy = numpy.mean(all_average_train_accuracy, axis=0)
